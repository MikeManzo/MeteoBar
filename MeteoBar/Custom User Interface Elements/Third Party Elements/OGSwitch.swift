...
///
/// [iOS-inspired Switch Control for macOS](https://github.com/OskarGroth/OGSwitch)
///
@IBDesignable public class OGSwitch: NSControl {
    ...
    internal func rectForKnob() -> CGRect {
        ...
        if (!hasDragged && !isOn) || (hasDragged && !isDraggingTowardsOn) {
            xStart = kBorderLineWidth
        } else {
            xStart = bounds.width - CGFloat(width) - kBorderLineWidth
        }
        
        return CGRect(x: xStart, y: kBorderLineWidth, width: CGFloat(width), height: height)
    }
